...
class DataExporterTest {
    ...
    fun setup() {
        ...
        exportDir = File.createTempFile("test", "dir").apply {
            delete()
            mkdirs()
        }
        every { context.getExternalFilesDir(null) } returns exportDir.parentFile
    ...
    fun `exportToJson creates valid JSON file with data`() = runTest {
        ...
        assertThat(exportData.sessions).hasSize(1)
        assertThat(exportData.sessions[0].packageName).isEqualTo("com.example.app")
        assertThat(exportData.sessions[0].appLabel).isEqualTo("Example App")
        assertThat(exportData.batterySamples).hasSize(1)
        assertThat(exportData.deviceEvents).hasSize(1)
    ...
    @Test
    fun `exportToJson with anonymize flag anonymizes sensitive data`() = runTest {
        // Given
        val startTime = 1000L
        val endTime = 2000L
        ...
        assertThat(exportData.sessions[0].packageName).startsWith("app.")
        assertThat(exportData.sessions[0].packageName).isNotEqualTo("com.example.app")
        assertThat(exportData.sessions[0].appLabel).isEqualTo("App")
        assertThat(exportData.sessions[0].locationLatitude).isNull()
        assertThat(exportData.sessions[0].locationLongitude).isNull()
    }
    
    @Test
    fun `exportSessionsToCsv creates valid CSV file`() = runTest {
        // Given
        val startTime = 1000L
        val endTime = 2000L
    ...
    @Test
    fun `exportToJson handles repository exceptions`() = runTest {
        // Given
        val startTime = 1000L
        val endTime = 2000L
