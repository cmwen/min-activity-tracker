 ...
 */
private fun AppError.getUserMessage(): String =
    when (this) {
        is AppError.DatabaseError.ConnectionError ->
            "Database connection failed. Please try again."
        is AppError.DatabaseError.CorruptionError ->
            "Data corruption detected. The app may need to reset its data."
        is AppError.DatabaseError.MigrationError ->
            "Database upgrade failed. Please restart the app."
        is AppError.PermissionError.UsageStatsNotGranted ->
            "Usage stats permission is required to track app usage."
        is AppError.PermissionError.LocationPermissionDenied ->
            "Location permission is required for location tracking."
        is AppError.PermissionError.NotificationAccessDenied ->
            "Notification access is required to monitor notifications."
        is AppError.DataCollectionError.ServiceUnavailable ->
            "Service is currently unavailable."
        is AppError.DataCollectionError.DataCorrupted ->
            "Data corruption detected."
        is AppError.UnknownError -> this.msg
    }

private fun AppError.getIcon(): ImageVector =
    when (this) {
